...
#include <sys/socket.h> // 用于 UDP 远程日志
#include <netinet/in.h> // 用于 UDP 远程日志
#include <arpa/inet.h>  // 用于 UDP 远程日志
...
public:
    ...
    void enableSyslog(const std::string& ident, int facility);
...
private:
    ...
    std::map<LogLevel_en, bool> logLevelEnabled = {
