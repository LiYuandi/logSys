...
{
    if (!logLevelEnabled[level]) return; // 如果该日志等级未启用，直接返回
...
void Logger::writeToRemote(const std::string& message)
{
    ...
    sendto(sockfd, message.c_str(), message.size(), 0, (struct sockaddr*)&serverAddr, sizeof(serverAddr)); // 发送日志消息
}
...
{
    std::lock_guard<std::mutex> lock(mutex);
    if (logLevelEnabled.find(level) != logLevelEnabled.end())
    {
        logLevelEnabled[level] = enable;
    }
}
...
{
    std::lock_guard<std::mutex> lock(mutex);
    for (auto& [lvl, enabled] : logLevelEnabled)
    {
        if (lvl >= level)
        {
            enabled = enable;
...
// 启用 syslog 支持
void Logger::enableSyslog(const std::string& ident, int facility)
{
    std::lock_guard<std::mutex> lock(mutex);
    useSyslog = true;
    syslogIdent = ident;
